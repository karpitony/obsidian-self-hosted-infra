...
강제 재인증(웹훅 링크 재발송)이 필요하면 아래 명령을 사용하세요.

* `uv run cli.py auth --force`

봇 실행 후 디스코드에서 슬래시 명령으로 제어합니다.

* `/status`: 봇 상태 확인
* `/auth`: 구글 드라이브 인증 프로세스 실행 (기존 토큰 유효 시 링크 미발송)
* `/auth force:true`: 웹훅 인증 링크 강제 재발송
* `/backup`: 즉시 백업 실행
